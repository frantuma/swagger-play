 ...
 */
package play.modules.swagger.routes
...
object RoutesFileParser {
  ...
  /**
    ...
    */
  def parse(classloader: ClassLoader, filename: String, prefix: String): Either[Seq[RoutesCompilationError], List[(String,Rule)]] = {
    val routesContent =  scala.io.Source.fromInputStream(classloader.getResourceAsStream(filename))(Codec("UTF-8")).mkString
    parseContent(routesContent, new File(filename)) match {
      case Right(rules) =>
        val eithers: List[Either[Seq[RoutesCompilationError], List[(String,Rule)]]] = rules.map {
          ...
          case include:Include => // load include route
            ...
            val route= include.router.replaceFirst("\\.Routes",".routes")
            parse(classloader,route,include.prefix+"/")
        }
